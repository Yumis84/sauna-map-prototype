// Чистая верхняя часть карточки + дополнительные фото из 101sauna.ru. v7.8
(function(){
  ...
  style.textContent=`
    ...
    .twogis-v4,.yandex-photos-v7{display:none!important}
  `;
  ...
  function cleanupUi(){
    document.querySelectorAll('[data-f="24/7"],.twogis-v4,.yandex-photos-v7').forEach(el=>el.remove());
  }

  merge101Photos();

  if(document.readyState==='loading'){
    document.addEventListener('DOMContentLoaded',cleanupUi,{once:true});
  }else cleanupUi();

  new MutationObserver(()=>requestAnimationFrame(cleanupUi)).observe(document.documentElement,{childList:true,subtree:true});
})();
