// Надёжное закрытие нижней карточки свайпом для Android/iOS. v6
(function(){
...
// Галерея карточки: один счётчик, который следует за фактически видимым слайдом.
(function galleryIndicatorFix(){
  const style=document.createElement('style');
  style.textContent=`
    .gallery-dots{display:none!important}
    .gallery-count{left:50%!important;right:auto!important;bottom:14px!important;transform:translateX(-50%);min-width:48px;text-align:center;font-weight:800;pointer-events:none}
  `;
  document.head.appendChild(style);

  const bound=new WeakSet();

  function visibleIndex(track){
  ...
  function bind(track){
    ...
    if(!hero)return;
    hero.querySelectorAll('.gallery-dots').forEach(el=>el.remove());
    const count=hero.querySelector('.gallery-count');
    ...
    const update=()=>{
      raf=0;
      const idx=visibleIndex(track);
      count.textContent=`${idx+1} / ${total}`;
      count.dataset.index=String(idx);
    };
    const schedule=()=>{
      if(raf)return;
    ...
    track.addEventListener('touchend',()=>setTimeout(update,20),{passive:true});
    track.addEventListener('pointerup',()=>setTimeout(update,20),{passive:true});
    if('onscrollend' in track)track.addEventListener('scrollend',update,{passive:true});
    new ResizeObserver(schedule).observe(track);
    update();
  }

  function scan(){
    document.querySelectorAll('.gallery-track').forEach(bind);
  }

  const sheet=document.getElementById('sheet');
  if(sheet)new MutationObserver(()=>requestAnimationFrame(scan)).observe(sheet,{childList:true,subtree:true});
